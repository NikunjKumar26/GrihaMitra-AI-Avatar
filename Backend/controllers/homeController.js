...
exports.approveMember = async (req, res) => {
  try {
    ...
    if(userToVerify) {
      ...
      // Automatically enroll approved user as a FamilyMember with provided details
      try {
        const { name, role, age, preferredLanguage, voiceProfile, routineProfile, preferences } = req.body;
        const FamilyMember = require('../models/FamilyMember');
        const RoutineHistory = require('../models/RoutineHistory');

        // Check if name is unique in this home
        let uniqueName = name || userToVerify.name;
        let nameExists = await FamilyMember.findOne({ home: home._id, name: new RegExp(`^${uniqueName}$`, 'i') });
        let counter = 1;
        while (nameExists) {
          uniqueName = `${name || userToVerify.name} (${counter})`;
          nameExists = await FamilyMember.findOne({ home: home._id, name: new RegExp(`^${uniqueName}$`, 'i') });
          counter++;
        }
        ...
        const newFamilyMember = new FamilyMember({
          ...
          voiceProfile: voiceProfile || 'Aditi',
          avatarImage: role === 'Father' ? '👨' : role === 'Mother' ? '👩' : role === 'Grandmother' ? '👵' : role === 'Student' ? '🧑‍🎓' : '👤',
          routineProfile: routineProfile || (role === 'Father' ? 'Office Routine' : role === 'Mother' ? 'Cooking Routine' : role === 'Grandmother' ? 'Pooja Routine' : role === 'Student' ? 'Study Routine' : 'Dynamic Routine'),
        ...
        // Log to RoutineHistory
        await RoutineHistory.create({
          memberId: newFamilyMember._id,
          activity: `Successfully joined house as a family member with role '${role || 'Guest'}'`,
          room: 'System Configuration',
          action: 'JOIN_HOUSE'
        });
      } catch (err) {
        console.error('Error auto-creating FamilyMember on approval:', err.message);
      }
    }
